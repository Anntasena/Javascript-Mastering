...
//$ TIMER LOGOUT FUNCTION
const startLogOutTimer = function () {
  const tick = function () {
    const min = String(Math.trunc(time / 60)).padStart(2, 0);
    const sec = String(time % 60).padStart(2, 0);
    //@ callback, menampilkan sisa waktu di UI
    labelTimer.textContent = `${min}:${sec}`;
    //@ saat timer 0, timer berhenti dan melakukan loggout
    if (time === 0) {
      //@ menghilangkan interval
      clearInterval(timer);
      //@ menghilangkan UI
      labelWelcome.textContent = "Log in to get started";
      containerApp.style.opacity = 0;
    }
    //@ mengurangi waktu setiap detiknya
    time--;
  };
  //# mengatur waktu menjadi 120 detik
  let time = 30;

  //# memanggil timer setiap detik
  tick();
...
btnLogin.addEventListener("click", function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    updateUI(currentAccount);
    //@ memulai timer saat login dan menghilangkan timer saat ganti akun
